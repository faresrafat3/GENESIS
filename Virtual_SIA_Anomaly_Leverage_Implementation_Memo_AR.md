# Virtual-SIA - مذكرة تنفيذ الاستفادة من الشذوذ
...
الدالة `compute_anomaly_severity_score()` في `virtual_sia/runtime/anomaly_runtime/service.py`:
...
الدالة `verify_output_anomaly_aware()` في `virtual_sia/runtime/verification_runtime/service.py`:
...
الدالة `choose_tier_anomaly_aware()` في `virtual_sia/runtime/economy_control/router.py`:
...
الدالة `should_escalate_anomaly_aware()` في `virtual_sia/runtime/economy_control/escalation.py`:
...
في `virtual_sia/runtime/pipeline/minimal_run.py`:
